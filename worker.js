...
const KV_KEYS = {
  ...
  tgaDts:            'tga_dts_v1',             // DTS 일별 TGA 잔고 (GitHub Actions)
};
const KV_TTL = {
  liq:        7200,
  yieldsHist: 86400,
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path.startsWith('/ici-raw'))       return await iciRaw();    // 디버그 (ICI 파싱 점검용)
...
async function newsTransDebug(env) {
  ...
  result.gemini_note = '번역 테스트는 뉴스 패널 강제새로고침으로 확인 (API 호출 절약)';

  return new Response(JSON.stringify(result, null, 2), { headers: CORS });
}

// ── 번역 캐시 초기화 (/news-trans-flush) ───────────────────────
// 기존 번역 캐시 삭제 → 다음 /news?force=1 시 Gemini로 재번역
